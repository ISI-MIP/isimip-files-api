...
def test_missing_country(client):
    ...
    assert response.json.get('errors') == {
        'operations': ['country is missing for operation "select_country"']
    }
...
def test_wrong_country(client):
    ...
    assert response.json.get('status') == 'error'
    assert response.json.get('errors') == {
        'operations': ['country not in the list of supported countries (e.g. deu) for operation "select_country"']
    }
